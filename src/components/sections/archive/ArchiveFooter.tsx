import { anton, nihonium } from "@/fonts";
...
const ArchiveFooter = ({ activeCategory }: ArchiveFooterProps) => {
  ...
  return (
    <div className="pointer-events-none fixed bottom-0 left-0 z-50 flex w-full flex-col items-center justify-between gap-2 px-4 py-4 uppercase mix-blend-difference md:flex-row md:items-center md:gap-0 md:px-6 lg:items-start lg:mix-blend-normal">
      <h1
        className={`${anton.className} text-glow text-7xl tracking-wide md:text-7xl lg:text-9xl`}
      >
        Archive
      </h1>
      <div className="relative hidden w-full flex-wrap items-center justify-start gap-x-6 gap-y-4 tracking-wider text-white sm:justify-center md:absolute md:left-1/2 md:w-auto md:-translate-x-1/2 md:justify-center md:gap-6 md:text-xl lg:flex lg:gap-10">
        <div
          className={`${nihonium.className} relative flex h-8 min-w-42 items-center justify-center border border-dashed border-gray-500 text-2xl`}
        >
          {categories.map((cat) => (
            <div
              key={cat.name}
